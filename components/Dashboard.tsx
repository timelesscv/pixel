  ...
  return (
    <div className="animate-fade-in flex flex-col items-start lg:items-center">
      <div className="mb-16 w-full lg:text-center">
        <h1 className="text-3xl md:text-5xl font-black mb-4 tracking-tighter leading-tight inline-block">
          <span className="bg-gradient-to-r from-white via-slate-300 to-slate-500 bg-clip-text text-transparent inline-flex items-center gap-3">
            {greeting}
          </span>
        </h1>
        <p className="text-slate-500 text-sm md:text-base font-medium tracking-tight opacity-70 mt-1">
        ...
        {Object.entries(settings.enabledCountries).map(([key, enabled]) => {
            ...
            return (
                <Card 
                  key={key}
                  title={key} 
                  flag={key === 'kuwait' ? "🇰🇼" : key === 'saudi' ? "🇸🇦" : key === 'jordan' ? "🇯🇴" : key === 'oman' ? "🇴🇲" : key === 'uae' ? "🇦🇪" : key === 'qatar' ? "🇶🇦" : "🇧🇭"} 
